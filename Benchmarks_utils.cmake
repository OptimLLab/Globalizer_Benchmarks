...
function (GLOBALIZER_BENCHMARKS_initialize)
  ...
  if (CMAKE_CXX_COMPILER_ID MATCHES "GNU")
	...
	message("${GLOBALIZER_GCC_VERSION}")
...
endfunction (GLOBALIZER_BENCHMARKS_initialize)
...
function (select_platform_depended_library arg_lib arg_envlibx86 arg_envlibx64)
  ...
  elseif (GLOBALIZER_BENCHMARKS_TARGET_ARCH MATCHES "x64")
...
function (get_platform_depended_library_path arg_out)
  if (WIN32)
    if (IS_MICROSOFT_COMPILER)
      set(${arg_out} lib/win/${GLOBALIZER_BENCHMARKS_VC_VERSION}/${GLOBALIZER_BENCHMARKS_TARGET_ARCH} PARENT_SCOPE)
...
macro(print_build_config)
  ...
  message("Target arch: " ${GLOBALIZER_BENCHMARKS_TARGET_ARCH})
...
# GLOBALIZER_BENCHMARKS_define_problem(<name> [<USE_CONF> <CUDA> <LINK_LIBS> <libs list> <INCLUDE_DIRS> <dirs list>
#                         <COMPILE_OPTIONS> <options list>])
macro(GLOBALIZER_BENCHMARKS_define_problem _name)

  set(BUILD_PROBLEM "${_name}_build")
  option(${BUILD_PROBLEM} "Build test programs" OFF)

  if(${BUILD_PROBLEM} OR BUILD_ALL_TASK)
  
	#parse arguments
	set(options USE_CONF CUDA)
	set(multiValueArgs INCLUDE_DIRS LINK_LIBS COMPILE_OPTIONS)
	cmake_parse_arguments(GLOBALIZER_BENCHMARKS_define_problem "${options}" "" "${multiValueArgs}" ${ARGN})
	
	#set up current project
	set(PROJECT_NAME_STR ${_name})
	if(${GLOBALIZER_BENCHMARKS_USE_OneAPI})
		project (${PROJECT_NAME_STR} CXX)
	else()
		project (${PROJECT_NAME_STR})
	endif()
	
	set(PROBLEM_OUTPUT_DIRECTORY ${GLOBALIZER_BENCHMARKS_OUTPUT_DIRECTORY})
	
	include_directories(${CMAKE_CURRENT_SOURCE_DIR})
	include_directories(${GLOBALIZER_BENCHMARKS_COMMON_SRC_DIR})
	file(GLOB SRC_FILES ${CMAKE_CURRENT_SOURCE_DIR}/*.cpp ${CMAKE_CURRENT_SOURCE_DIR}/*.h
	${CMAKE_CURRENT_SOURCE_DIR}/*.hpp)
	
	if(${GLOBALIZER_BENCHMARKS_define_problem_CUDA})
	...
	target_link_libraries(${PROJECT_NAME_STR} ${GLOBALIZER_BENCHMARKS_define_problem_LINK_LIBS})
	target_include_directories(${PROJECT_NAME_STR} PUBLIC ${GLOBALIZER_BENCHMARKS_define_problem_INCLUDE_DIRS})

  endif(${BUILD_PROBLEM} OR BUILD_ALL_TASK)
endmacro()
